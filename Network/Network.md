...
여러 가지 종류가 있지만, 각각의 솔루션은 Inline, Out Of Path, Proxy 중 하나라는 사실에는 변함이 없다.

* 수동적 보안 인프라

  * "정책"을 중요시. 즉, 단편적인 조건을 보고 패킷(혹은 스트림)을 거를 건지 판단한다.

  * OSI 7계층 중 어느 수준의 데이터를 보느냐에 따라 솔루션을 분류할 수 있다.

  * L2~L3 계층

    * NAC

      * L2,L3 계층의 데이터(IP, PORT(?))를 확인하여 차단 조건에 부합하면 차단한다

        => L2 인터페이스를 물리적으로 차단시키거나, HTTP Redirect를 행하는 방법으로 차단이 이루어진다.

      * 일반적으로는, 별도의 인증 서버로 트래픽을 보내서(보통 RADIUS 인증체계 활용), 인증을 통과하면 더 상위 OSI layer로 트래픽을 보낸다.
  ...
  * L3~L4 계층
    ...
    * Packet Filtering F/W
      ...
      * ** Application 수준의 F/W 도 존재한다. (이를 WAF 라고 부른다.)

        => HTTP 트래픽을 모니터링한다.

    * VPN (G to G)

  * 위의 NAT, F/W, VPN이 수행하는 기능을 몽땅 한 기기에 넣을 수도 있으며, 이를 "UTM" 이라고 부른다.

* 능동적 보안 인프라

  * RULE을 중요시

    => 단순한 정책적인 부분도 체크하며, DPI까지 시행 (즉, appplication 수준의 데이터까지 다 뜯어봄.)

    => S/W 계층에 해당하는 데이터는 다 뜯어본다고 볼 수 있다.

  * 발전 과정이 존재

    * 초창기에는 IPS(inline), NIDS(out Of path)를 활용함

      * 서버가 정해둔 특정 패턴(차단요건) 확인을 위해, 시그니처 분석까지 시행
      * 그러나, 이는 파일 형태의 악성코드를 탐지 못 한다는 한계점 존재
    ...
    * 그러나 MPS도 한계가 있다고 한다.
      ...
      * CLOUD(평판 조회 시스템)에 해당 파일이 안전한지 확인.

        => MPS 뿐만 아니라, IPS도 CLOUD를 활용

    * 이 과정에서, LOG가 어마어마하게 쌓인다.

      * 각 보안 장비의 Health check 등등을 총괄하는 ESM (Enterprise security manager)등장

  * 위처럼 보안 장비 관련해서는 복잡한 구조로 이루어져 있으므로, 서버에 장애가 발생할 때, 보안 장비의 이슈로 인해 장애가 발생하는 경우가 존재한다.
